...
const getRouteCoordinates = async (activityId, compressionLevel = 0.0001) => {
  const [coords] = await getMySQLConnection().query(`
    SELECT * FROM route_coordinates_n WHERE
    activityId = ? AND compression_level = ?
  `, [activityId, compressionLevel]);
