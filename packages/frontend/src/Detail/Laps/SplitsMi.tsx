    ...
    <div className="mt-4 card">
      <Surface className="overflow-x-auto">
        <table>
          <thead>
            <tr>
              <th colSpan={6} className="raised-1 bg-neutral-800 text-white text-center">Mile Splits</th>
            </tr>
            ...
            {details.splits_standard.map((split, ix) => {
              return (
                <tr key={split.split} className={`text-right text-white bg-neutral-700 ${ix % 2 === 0 ? 'sunken-1' : ''}`}>
                  <td>{split.split}</td>
                  <td><DurationDisplay numSeconds={split.elapsed_time} /></td>
                  <td>{convertMetersToMiles(split.distance)} <small>mi</small></td>
                  <td>
                    <DurationDisplay
                      numSeconds={Math.floor((1 / convertMetersToMiles(split.distance)) * split.elapsed_time)} units={['', ':']}
                    />
                  </td>
